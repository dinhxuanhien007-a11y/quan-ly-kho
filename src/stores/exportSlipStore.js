// src/stores/exportSlipStore.js (Đã sửa lỗi hiển thị Dropdown)
...
            }),
            
            // --- HÀM XỬ LÝ KẾT QUẢ TÌM KIẾM SẢN PHẨM (SOFT ALLOCATION LOGIC) ---
...
try {
    // 1. CHỈ TRUY VẤN CÁC LÔ CÓ TỒN THỰC TẾ > 0
...
                    } else {
        ...
        for (const lot of foundLots) {
            ...
            if (lotAggregator.has(lotKey)) {
                ...
                existing.quantityAllocated += allocatedQty;
                existing.availableQty += availableQty; // Tồn Khả Dụng đã gộp
                                
...
                            } else {
                // KHỞI TẠO LÔ MỚI TRONG AGGREGATOR
                lotAggregator.set(lotKey, { 
                    ...lot, 
                    id: lotKey, 
...
                        
                        // Tính tổng tồn khả dụng
                        const totalAvailable = finalLots.reduce((sum, lot) => sum + lot.availableQty, 0);

                        // Nếu sau khi lọc Allocation mà hết hàng, đánh dấu là OutOfStock
                        if (aggregatedLots.length === 0 || totalAvailable <= 0) outOfStock = true;
                    }
...
                        items: state.items.map((item, i) => {
                            if (i === index) {
                                // Tính tổng tồn khả dụng của TẤT CẢ các lô đã gom nhóm
                                const totalAvailable = finalLots.reduce((sum, lot) => sum + lot.availableQty, 0);
                                
                                return {
                                    ...item,
...
                                    isFetchingLots: false,
                                    quantityAvailableForExport: totalAvailable, // Gán TỔNG TỒN KHẢ DỤNG
                                };
                            }
                            return item;
                        })

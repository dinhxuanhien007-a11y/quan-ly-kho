...
const NewExportPage = () => {
  ...
  const [items, setItems] = useState([{ 
      ...
      lotNumber: '',
      displayLotText: '',
      expiryDate: '', // Vẫn lưu HSD nhưng không hiển thị
      quantityRemaining: 0,
      quantityToExport: '', 
      notes: '' 
  }]);
  ...
  const getValidSlipData = () => {
    if (!customer) {
      alert('Vui lòng nhập thông tin Khách hàng / Nơi nhận.');
      return null;
    }
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      setItems([{ id: Date.now(), productId: '', productName: '', unit: '', packaging: '', storageTemp: '', availableLots: [], selectedLotId: '', lotNumber: '', displayLotText: '', expiryDate: '', quantityRemaining: 0, quantityToExport: '', notes: '' }]);
    } catch (error) {
    ...
    <div>
      ...
      <div className="form-section">
        <div className="form-row">
          <div className="form-group"><label>Ngày xuất</label><input type="text" value={exportDate} onChange={(e) => setExportDate(e.target.value)} /></div>
          <div className="form-group"><label>Khách hàng / Nơi nhận</label><input type="text" placeholder="Nhập tên khách hàng..." value={customer} onChange={e => setCustomer(e.target.value)} /></div>
        </div>
        <div className="form-group"><label>Diễn giải</label><textarea rows="2" placeholder="Ghi chú cho phiếu xuất..." value={description} onChange={e => setDescription(e.target.value)}></textarea></div>
      </div>

      <h2>Chi Tiết Hàng Hóa Xuất Kho</h2>
      {/* --- BỐ CỤC MỚI (ĐÃ BỎ CỘT HSD) --- */}

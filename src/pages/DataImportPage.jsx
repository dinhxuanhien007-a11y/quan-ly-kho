// src/pages/DataImportPage.jsx

import React, { useState } from 'react';
import { db } from '../firebaseConfig';
import { collection, writeBatch, doc, Timestamp, serverTimestamp } from 'firebase/firestore';
import { toast } from 'react-toastify';
import Papa from 'papaparse';
import { FiUpload, FiDownload, FiInfo } from 'react-icons/fi';
import { parseDateString } from '../utils/dateUtils';
import styles from '../styles/DataImportPage.module.css';
import { normalizeString, generateKeywords } from '../utils/stringUtils';

const DataImportPage = () => {
    const [importType, setImportType] = useState('inventory');
    const [pastedData, setPastedData] = useState('');
    const [isImporting, setIsImporting] = useState(false);
    const [importLog, setImportLog] = useState([]);

    const logMessage = (message, type = 'info') => {
        setImportLog(prev => [...prev, { message, type, time: new Date().toLocaleTimeString() }]);
    };

    const handleFileImport = (e) => {
        const file = e.target.files[0];
        if (!file) return;
        logMessage(`Bắt đầu đọc file: ${file.name}`);
        Papa.parse(file, {
            header: true,
            skipEmptyLines: true,
            complete: (results) => { processData(results.data); },
            error: (err) => {
                toast.error("Không thể đọc file CSV.");
                logMessage(`Lỗi đọc file: ${err.message}`, 'error');
            }
        });
        e.target.value = null;
    };

    const handlePasteImport = () => {
        if (!pastedData.trim()) return toast.warn("Vui lòng dán dữ liệu vào ô trống.");
        logMessage(`Bắt đầu đọc dữ liệu đã dán.`);
        Papa.parse(pastedData, {
            header: true,
            skipEmptyLines: true,
            complete: (results) => { processData(results.data); },
            error: (err) => {
                toast.error("Định dạng dữ liệu đã dán không hợp lệ.");
                logMessage(`Lỗi đọc dữ liệu: ${err.message}`, 'error');
            }
        });
    };

    const processData = async (data) => {
        if (!data || data.length === 0) {
            toast.warn("Không có dữ liệu hợp lệ để import.");
            logMessage("Không tìm thấy dòng dữ liệu nào.", 'warn');
            return;
        }

        setIsImporting(true);
        setImportLog([]);
        logMessage(`Phát hiện ${data.length} dòng. Bắt đầu xử lý loại: ${importType}...`);

        try {
            const MAX_BATCH_SIZE = 490;
            let batch = writeBatch(db);
            let operationCount = 0;
            let totalSuccess = 0;
            let skipped = 0;

            for (let i = 0; i < data.length; i++) {
                const row = data[i];
                const rowIndex = i + 2;

                if (importType === 'inventory' || importType === 'products_only') {
                    if (!row.productId || !String(row.productId).trim() || !row.productName || !String(row.productName).trim()) {
                        logMessage(`Bỏ qua dòng ${rowIndex}: Thiếu productId hoặc productName.`, 'warn');
                        skipped++;
                        continue;
                    }

                    const productId = String(row.productId).trim().toUpperCase();
                    const productRef = doc(db, 'products', productId);

                    const conversionFactor = Number(row.conversionFactor) || 1;
                    const misaConversionFactor = Number(row.misaConversionFactor) || 1;

                    const productData = {
                        productName: String(row.productName).trim(),
                        unit: String(row.unit || '').trim(),
                        packaging: String(row.packaging || '').trim(),
                        storageTemp: String(row.storageTemp || '').trim(),
                        manufacturer: String(row.manufacturer || '').trim(),
                        team: String(row.team || 'MED').trim(),
                        subGroup: String(row.subGroup || '').trim(),
                        conversionFactor,
                        misaConversionFactor,
                        misaUnit: String(row.misaUnit || '').trim(),
                        createdAt: serverTimestamp()
                    };
                    batch.set(productRef, productData, { merge: true });
                    operationCount++;

                    // Nếu là "products_only" thì không tạo lô tồn kho
                    if (importType === 'products_only') {
                        totalSuccess++;
                    } else {
                        // Chỉ tạo lô tồn kho nếu có số lượng
                        const quantityStr = String(row.quantityRemaining || '').replace(/[.,\s]/g, '');
                        if (quantityStr && !isNaN(Number(quantityStr)) && Number(quantityStr) > 0) {
                            const quantityNum = Number(quantityStr);
                            const lotNumber = String(row.lotNumber || '').trim() || null;
                            const expiryDate = parseDateString(row.expiryDate);

                            if (!expiryDate && row.expiryDate && String(row.expiryDate).trim()) {
                                logMessage(`Dòng ${rowIndex} (${productId}): Sai định dạng HSD, bỏ qua lô tồn kho.`, 'warn');
                            } else {
                                const inventoryRef = doc(collection(db, 'inventory_lots'));
                                batch.set(inventoryRef, {
                                    productId,
                                    productName: productData.productName,
                                    lotNumber,
                                    expiryDate: expiryDate ? Timestamp.fromDate(expiryDate) : null,
                                    importDate: Timestamp.now(),
                                    quantityImported: quantityNum,
                                    quantityRemaining: quantityNum,
                                    unit: productData.unit,
                                    packaging: productData.packaging,
                                    storageTemp: productData.storageTemp,
                                    team: productData.team,
                                    manufacturer: productData.manufacturer,
                                    subGroup: productData.subGroup,
                                    supplierName: 'Tồn đầu kỳ',
                                    notes: String(row.notes || '').trim()
                                });
                                operationCount++;
                            }
                        }
                        totalSuccess++;
                    }

                } else if (importType === 'partners') {
                    if (!row.partnerId || !row.partnerName) {
                        logMessage(`Bỏ qua dòng ${rowIndex}: Thiếu partnerId hoặc partnerName.`, 'warn');
                        skipped++;
                        continue;
                    }
                    const docId = String(row.partnerId).trim().toUpperCase();
                    const partnerName = String(row.partnerName).trim();
                    const creationDate = parseDateString(row.creationDate);

                    batch.set(doc(collection(db, 'partners'), docId), {
                        partnerName,
                        partnerType: String(row.partnerType || '').trim().toLowerCase() === 'customer' ? 'customer' : 'supplier',
                        createdAt: creationDate ? Timestamp.fromDate(creationDate) : Timestamp.now(),
                        partnerNameNormalized: normalizeString(partnerName),
                        searchKeywords: generateKeywords(partnerName)
                    }, { merge: true });
                    operationCount++;
                    totalSuccess++;
                }

                // Commit batch khi đầy hoặc xử lý xong
                if (operationCount >= MAX_BATCH_SIZE || i === data.length - 1) {
                    if (operationCount > 0) {
                        await batch.commit();
                        batch = writeBatch(db);
                        operationCount = 0;
                    }
                }
            }

            const msg = `Hoàn tất! Thành công: ${totalSuccess}/${data.length} mục${skipped > 0 ? `, bỏ qua: ${skipped}` : ''}.`;
            toast.success(msg);
            logMessage(msg, 'success');

        } catch (error) {
            console.error("Lỗi khi import dữ liệu: ", error);
            toast.error("Đã xảy ra lỗi trong quá trình import.");
            logMessage(`Lỗi nghiêm trọng: ${error.message}`, 'error');
        } finally {
            setIsImporting(false);
            setPastedData('');
        }
    };

    const downloadTemplate = () => {
        let headers, filename, sampleData;

        if (importType === 'partners') {
            headers = "partnerId*,partnerName*,partnerType,creationDate";
            filename = "mau_import_doi_tac.csv";
            sampleData = "NCC-01,CÔNG TY DƯỢC PHẨM ABC,supplier,25/12/2024\nKH-01,BỆNH VIỆN XYZ,customer,";
        } else if (importType === 'products_only') {
            headers = "productId*,productName*,unit,packaging,storageTemp,team,manufacturer,subGroup,conversionFactor,misaConversionFactor,misaUnit";
            filename = "mau_import_san_pham.csv";
            sampleData = "SP001,BÔNG CỒN ALKOCIDE,Hộp,100 miếng/hộp,Nhiệt độ phòng,MED,DentaLife,Spare Part,1,1,\nSP002,GĂNG TAY Y TẾ,Hộp,50 đôi/hộp,,MED,,,100,1,Hộp";
        } else {
            headers = "productId*,productName*,lotNumber,quantityRemaining*,expiryDate,unit,packaging,storageTemp,team,manufacturer,subGroup,conversionFactor,misaConversionFactor,misaUnit,notes";
            filename = "mau_import_san_pham_ton_kho.csv";
            sampleData = "SP001,BÔNG CỒN ALKOCIDE,L202501,100,31/12/2025,Hộp,100 miếng/hộp,Nhiệt độ phòng,MED,DentaLife,Spare Part,1,1,,Hàng ưu tiên\nSP002,GĂNG TAY Y TẾ,GT001,50,,Hộp,50 đôi/hộp,,MED,,,100,1,Hộp,";
        }

        const csvContent = "data:text/csv;charset=utf-8,\uFEFF" + headers + "\n" + sampleData;
        const link = document.createElement("a");
        link.setAttribute("href", encodeURI(csvContent));
        link.setAttribute("download", filename);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };

    const importTypeLabels = {
        inventory: 'Sản phẩm & Tồn kho',
        products_only: 'Chỉ khai báo Sản phẩm',
        partners: 'Đối tác'
    };

    return (
        <div className={styles.dataImportPage}>
            <div className="page-header">
                <h1>Import Dữ Liệu Hàng Loạt</h1>
            </div>

            <div className={styles.importContainer}>
                <div className={styles.importControls}>
                    <h3>1. Chọn loại dữ liệu</h3>
                    <div className={styles.importTypeSelector}>
                        <button onClick={() => setImportType('inventory')} className={importType === 'inventory' ? styles.active : ''}>
                            Sản phẩm & Tồn kho
                        </button>
                        <button onClick={() => setImportType('products_only')} className={importType === 'products_only' ? styles.active : ''}>
                            Chỉ khai báo Sản phẩm
                        </button>
                        <button onClick={() => setImportType('partners')} className={importType === 'partners' ? styles.active : ''}>
                            Đối tác
                        </button>
                    </div>

                    <h3>2. Chuẩn bị dữ liệu</h3>
                    <p>Dữ liệu cần ở định dạng CSV (UTF-8). Dùng file mẫu để đảm bảo đúng cấu trúc cột.</p>
                    <button onClick={downloadTemplate} className="btn-secondary" style={{ width: '100%' }}>
                        <FiDownload /> Tải File Mẫu ({importTypeLabels[importType]})
                    </button>

                    <div className={styles.importInstructions}>
                        <FiInfo />
                        <div>
                            <strong>Lưu ý quan trọng:</strong>
                            <ul>
                                <li>Cột có dấu <code>*</code> là bắt buộc.</li>
                                <li>Nếu <code>productId</code> chưa có, sản phẩm mới sẽ được tạo tự động.</li>
                                {importType === 'inventory' && <li>Mỗi dòng có số lượng sẽ tạo một lô tồn kho mới.</li>}
                                {importType === 'products_only' && <li>Chỉ khai báo thông tin sản phẩm, không tạo lô tồn kho.</li>}
                                <li>Nếu <code>partnerId</code> đã tồn tại, dữ liệu cũ sẽ bị ghi đè.</li>
                                <li>Định dạng ngày: <code>dd/mm/yyyy</code>.</li>
                                <li><code>conversionFactor</code>: hệ số quy đổi nội bộ (mặc định 1).</li>
                                <li><code>misaConversionFactor</code>: hệ số quy đổi sang đơn vị Misa (mặc định 1).</li>
                            </ul>
                        </div>
                    </div>
                </div>

                <div className={styles.importActions}>
                    <h3>3. Tải lên và thực hiện</h3>
                    <div className={styles.importMethod}>
                        <h4>Cách 1: Tải lên file .csv</h4>
                        <input type="file" accept=".csv" onChange={handleFileImport} disabled={isImporting} />
                    </div>
                    <div className={styles.importMethod}>
                        <h4>Cách 2: Dán dữ liệu từ Excel/Google Sheets</h4>
                        <textarea
                            rows="8"
                            placeholder="Dán dữ liệu vào đây (bao gồm cả dòng tiêu đề)"
                            value={pastedData}
                            onChange={(e) => setPastedData(e.target.value)}
                            disabled={isImporting}
                        />
                        <button onClick={handlePasteImport} className="btn-primary" disabled={isImporting} style={{ marginTop: '10px' }}>
                            <FiUpload /> {isImporting ? 'Đang import...' : 'Import từ dữ liệu đã dán'}
                        </button>
                    </div>
                    <div className={styles.importLogContainer}>
                        <h4>Nhật ký Import</h4>
                        <div className={styles.importLog}>
                            {importLog.length === 0
                                ? <p>Chưa có hoạt động nào.</p>
                                : importLog.map((log, index) => (
                                    <p key={index} className={`${styles.logItem} ${styles[`log-${log.type}`]}`}>
                                        <span>[{log.time}]</span> {log.message}
                                    </p>
                                ))
                            }
                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
};

export default DataImportPage;
